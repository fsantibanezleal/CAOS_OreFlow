...
must be too: ONE file carries both languages, tagged `l-en` and `l-es` at the same coordinates, and
the shell stylesheet shows exactly one. Two files per diagram would be two things to keep in step, and
the one that is not on screen is the one that goes stale.
