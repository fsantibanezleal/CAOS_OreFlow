...
async function get<T>(path: string): Promise<T> { const response = await fetch(`${base}data/${path}`); if (!response.ok) throw new Error(`${response.status} ${path}`); return response.json() as Promise<T>; }
