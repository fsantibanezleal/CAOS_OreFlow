...
export const SEPARATION: Topic[] = [
  ...
  },
  {
    ...
    equations: [
      { tex: r`\begin{gathered} E_g(d) = E_{max}\left(1 - e^{-(d/x_g)^2}\right) \\ m = f + (1 - b\,E_{eff})\,C\,p \end{gathered}`, caption: { en: 'Gravity recovery of liberated gold by size on the bleed b, and the mill feed m with the gravity unit in the loop.', es: 'Recuperación gravimétrica del oro liberado por tamaño en la purga b, y la alimentación al molino m con la unidad gravimétrica en el circuito.' } },
    ],
    table: {
      head: [{ en: 'Parameter', es: 'Parámetro' }, { en: 'Value', es: 'Valor' }, { en: 'Source', es: 'Fuente' }],
      rows: [
  ...
  },
  {
    ...
    equations: [
      { tex: r`\begin{gathered} y_{des}(d) = R_b + (1 - R_b)\left(1 - e^{-\ln 2\,(d/d_{des})^{m}}\right) \\ d_{des} \le \tfrac12\,P_{80}^{*} \end{gathered}`, caption: { en: 'Desliming partition to underflow, and the contract rule that keeps the cut at most half the grind target P80*.', es: 'Partición del deslamado a la descarga, y la regla del contrato que mantiene el corte bajo la mitad del objetivo de molienda P80*.' } },
    ],
    table: {
      head: [{ en: 'Parameter', es: 'Parámetro' }, { en: 'Value', es: 'Valor' }, { en: 'Source', es: 'Fuente' }],
      rows: [
