...
export const METHODS: Topic[] = [
  {
    ...
    equations: [
      ...
      { tex: r`\begin{gathered} R_N = \int_0^\infty R(t)\,\frac{t^{N-1}e^{-t/\tau_c}}{\tau_c^N (N-1)!}\,dt \\ R_{1,N} = A\left[1 - (1 + k\tau_c)^{-N}\right] \end{gathered}`, caption: { en: 'Projection R_N of a batch curve to a bank of N cells of residence tau_c, and the first-order closed form R_1,N.', es: 'Proyección R_N de una curva batch a un banco de N celdas de residencia tau_c, y la forma cerrada de primer orden R_1,N.' } },
    ],
    limits: [
  ...
  },
  {
    ...
    equations: [
      { tex: { en: r`\begin{gathered} \max_{u \in U}\ \dot m_{pay}(u) \\ \text{s.t.}\quad g(u) \ge g_{min},\quad P_{req}(u) \le P_{inst},\quad w(u) \le w_{max} \end{gathered}`, es: r`\begin{gathered} \max_{u \in U}\ \dot m_{pay}(u) \\ \text{s.a.}\quad g(u) \ge g_{min},\quad P_{req}(u) \le P_{inst},\quad w(u) \le w_{max} \end{gathered}` }, caption: { en: 'Recovered payable over the operating decisions u in the contract envelope U, under the grade, power and water constraints.', es: 'Pagable recuperado sobre las decisiones de operación u en la envolvente U del contrato, con las restricciones de ley, potencia y agua.' } },
      { tex: r`s_c = \sigma_c\,\frac{v_c - \ell_c}{\ell_c},\qquad \min_c s_c \ge -10^{-6}`, caption: { en: 'Constraints as relative slacks: v the value, ℓ the limit, and the sign σ chosen so that a met constraint has s ≥ 0; a point is feasible when no slack falls below -1e-6.', es: 'Restricciones como holguras relativas: v el valor, ℓ el límite, y el signo σ elegido para que una restricción cumplida tenga s ≥ 0; un punto es factible cuando ninguna holgura cae bajo -1e-6.' } },
    ],
    limits: [
