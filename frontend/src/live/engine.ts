...
export function simulateLive(p: Params, caseId: string): Trace {
  ...
  const cut = clamp(0.34 * p.classifier_cut_um / Math.sqrt(Math.max(0.25, p.water_m3_t / 2)) * Math.sqrt(2.65 / Math.max(p.density_t_m3, 1.2)) * Math.pow(Math.max(p.feed_tph, 1) / 500, 0.08), 8, p.feed_p80_um);
  const bins = ground.map((v, i) => Math.max(0, v - (ground[i - 1] ?? 0)));
  bins[bins.length - 1] += Math.max(0, 1 - bins.reduce((a, b) => a + b, 0));
  const overflowBins = bins.map((mass, i) => mass / (1 + Math.exp(Math.min(60, Math.max(-60, (sizes[i] - cut) / Math.max(cut * 0.16, 1))))));
  const overflowFraction = overflowBins.reduce((a, b) => a + b, 0);
  let cumulative = 0;
  const overflow = overflowBins.map(mass => (cumulative += mass) / Math.max(overflowFraction, 1e-12));
  ...
  const recovery = Array.from({ length: 96 }, (_, i) => overflowFraction * 0.97 * (1 - Math.exp(-rate * p.flotation_time_min * i / 95)));
  const recoveryPct = recovery[95] * 100;
  const massPull = Math.min(clamp(0.018 + 0.085 * overflowFraction + 0.000015 * p.reagent_gpt, 0.015, 0.18), overflowFraction * 0.95);
  const concentrateTph = p.feed_tph * massPull;
  const grade = 100 * (p.feed_tph * p.feed_grade_pct / 100 * recovery[95]) / Math.max(concentrateTph, 1e-9);
