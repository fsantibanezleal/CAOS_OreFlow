...
const rr = (x: number, p80: number, slope = 0.72) => 1 - Math.exp(-Math.pow(x / (p80 / Math.pow(-Math.log(0.2), 1 / slope)), slope));
...
export function simulateLive(p: Params, caseId: string): Trace {
  ...
  const overflow = ground.map((v, i) => v / (1 + Math.exp(Math.min(60, Math.max(-60, (sizes[i] - cut) / Math.max(cut * 0.16, 1))))));
  ...
  const recovery = Array.from({ length: 96 }, (_, i) => 0.97 * (1 - Math.exp(-rate * p.flotation_time_min * i / 95)));
  const recoveryPct = recovery[95] * 100;
  const overflowFraction = 0.05;
  const massPull = clamp(0.018 + 0.085 * overflowFraction + 0.000015 * p.reagent_gpt, 0.015, 0.18);
