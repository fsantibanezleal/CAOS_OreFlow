...
| `TICKS_PROBE` | the count each categorical chart declares on its host (`data-ticks-cut`): its labels that do not fit their category once wrapped; the canvas is out of every page probe's reach, so the chart measures its own labels, and the record keeps how many charts declared | in Spanish at 1280x800 the four Sobol factor names ran into each other; squeezed to 220 px, the fixed chart declares all four |
