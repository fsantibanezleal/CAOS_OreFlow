...
| GM-05 | THE web SHALL distinguish measured locked-cycle test recovery from authored simulator recovery and SHALL expose observed-versus-predicted values and spatial sample positions in a selectable view. | `frontend/gate.mjs`: every tab of the Benchmark page, the measured-lane tables loaded, in both themes and languages; every screenshot read. |
