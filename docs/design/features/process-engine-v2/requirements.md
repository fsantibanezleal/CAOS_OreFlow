...
| ID | Requirement | Named gate |
| --- | --- | --- |
| PE-26 | THE first-order, Kelsall, Klimpel and compressed/stretched exponential records SHALL be least-squares fits to the engine's batch curve, each reporting parameters, fit RMSE and the plant-bank projection under the bank residence distribution. | `tests/test_kinetics.py::test_fits_and_bank_projection` |
