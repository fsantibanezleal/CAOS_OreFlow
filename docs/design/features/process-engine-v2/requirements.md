...
| PE-40 | THE Case view SHALL present every case's context in the order problem, components and variables, formalization, scope and assumptions, what each variant shows, and how to read the views, with every number read from the artifact; every qualitative claim of the authored context SHALL hold on the baked results. | `frontend/src/test/case-claims.test.ts`; browser gate case/Context screenshots |
