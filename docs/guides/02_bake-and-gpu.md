...
| cases | `cases/<case>.json`: every variant's trace, optimization and uncertainty records, the nominal Sobol record | 326.6 s on 12 workers |
| learning | `learning.json`, `models/process_surrogate.onnx`, `process_guard.onnx`, `process_surrogate.json` | 1971.9 s, on CUDA |
