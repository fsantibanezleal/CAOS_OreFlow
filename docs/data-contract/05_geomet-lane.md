...
**Paired uncertainty of the ranking.** With 52 tests in 29 holes, point estimates of RMSE do not
rank four models. Each protocol therefore carries a paired bootstrap over complete holes (2000
resamples, seed 42): every resample draws holes with replacement and scores every model on the same
rows, so the difference between two models carries their shared sampling noise once. The record
