...
def _params(cid: str, **kwargs: float) -> FeedParams:
    ...
    return FeedParams(case_id=cid, seed=42, **defaults)


CASES = (
    Case("copper_porphyry_soft", "liberation", "Soft copper porphyry", "A competent but relatively soft sulphide feed where liberation is purchased with modest grinding energy.", _params("copper_porphyry_soft", feed_grade_pct=0.74, hardness_kwh_t=11.0, grind_p80_um=165.0), _variants(), "recovery gains flatten once liberated surface is available", "authored scenario; model calibration required"),
    Case("copper_porphyry_hard", "liberation", "Hard copper porphyry", "A high-work-index porphyry that makes the energy-liberation trade-off visible.", _params("copper_porphyry_hard", feed_grade_pct=0.52, hardness_kwh_t=20.5, grind_p80_um=190.0), _variants(), "Bond energy dominates cost and coarse grinding penalises recovery", "authored scenario; model calibration required"),
    Case("gold_free_milling", "liberation", "Free-milling gold", "A gravity plus flotation proxy with high density and a broad particle-size response.", _params("gold_free_milling", feed_grade_pct=0.034, density_t_m3=3.35, hardness_kwh_t=15.5, grind_p80_um=125.0, classifier_cut_um=95.0, reagent_gpt=88.0), _variants(), "grade-recovery is sensitive to very low head grade", "authored scenario; model calibration required"),
    Case("iron_magnetite_fine", "classification", "Fine magnetite concentration", "A dense mineral circuit in which hydrocyclone cut size and water balance shape the overflow.", _params("iron_magnetite_fine", feed_grade_pct=28.0, density_t_m3=3.9, feed_p80_um=9_000.0, grind_p80_um=95.0, classifier_cut_um=72.0, reagent_gpt=34.0), _variants(), "classification sharpness drives mass pull more than reagent dosage", "authored scenario; model calibration required"),
    Case("nickel_laterite", "classification", "Nickel laterite", "A clay-rich feed with high water demand and an intentionally imperfect classifier response.", _params("nickel_laterite", feed_grade_pct=1.42, density_t_m3=2.45, hardness_kwh_t=8.2, feed_p80_um=12_000.0, grind_p80_um=240.0, classifier_cut_um=165.0, water_m3_t=3.6, reagent_gpt=190.0), _variants(), "water and fines impose a throughput-quality compromise", "authored scenario; model calibration required"),
    Case("phosphate_clay", "classification", "Phosphate with clay slimes", "A desliming-sensitive feed for examining overflow losses and noisy proxy signals.", _params("phosphate_clay", feed_grade_pct=11.4, density_t_m3=2.2, hardness_kwh_t=6.4, feed_p80_um=10_500.0, grind_p80_um=310.0, classifier_cut_um=210.0, water_m3_t=4.5, reagent_gpt=120.0), _variants(), "coarse grind protects water use but leaves liberated fines behind", "authored scenario; model calibration required"),
